...
AI Agents need this skill to execute raw, single API calls to configured LLMs without invoking the agent's main planning loop. Useful for testing prompts, performing comparative testing, or sending text directly to models.

## ⚙️ How It Works

The workflow procedure is:

1. **Parameters**: Read the target model name and prompt text.
2. **Execute**: Launch `llm_call.py` passing `--model`, `--prompt` using the appropriate python launcher based on the operating system.
3. **Output**: Return the raw model response directly.
...
* **Windows Python launcher**: Do not run `python3` on Windows systems. Use `py -3` or the resolved alias instead.
