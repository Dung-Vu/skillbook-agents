...
AI Agent cần kỹ năng này khi lập trình viên yêu cầu thử nghiệm trực tiếp một prompt thô, so sánh câu trả lời giữa các phiên bản mô hình khác nhau hoặc thực hiện các cuộc gọi API riêng lẻ không phụ thuộc vào luồng suy nghĩ mặc định.

## ⚙️ Cách Hoạt Động

Quy trình hoạt động:

1. **Đọc đầu vào**: Xác định mô hình đích (như `--model`) và nội dung prompt.
2. **Thực thi**: Chạy script `llm_call.py` bằng trình khởi chạy Python phù hợp theo hệ điều hành.
3. **Trả kết quả**: Xuất trực tiếp kết quả phản hồi thô từ mô hình.
...
* **Lỗi môi trường Python trên Windows**: Không được giả định lệnh `python3` hoạt động trên Windows. Hãy sử dụng trình khởi chạy `py -3` hoặc `python`.
